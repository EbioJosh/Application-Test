...
class AuthCoordinator:
    ...
    def set_hardware_components(self, rfid_reader, keypad):
        """
        Assign hardware components after they are initialized.
        """
    ...
    def _timeout_monitor(self):
        """Background thread that resets the session after inactivity."""
        while True:
            if self.state != "IDLE":
    ...
    def handle_rfid_detected(self, rfid_uid):
        ...
        with self.lock:
            if self.state != "IDLE":
                print(f"[COORDINATOR] Blocked RFID read: state={self.state}")
                return
    ...
    # -----------------------------
    # Motor cash dispensing
    # -----------------------------
    def dispense_cash(self, amount):
        """
        Dispense cash via motor. Amount is in PHP.
        Each 500 PHP = 1 motor run.
        """
        if not self.motor:
            print("[MOTOR] Motor not initialized!")
            return

        try:
            amount = int(amount)
        except ValueError:
            print(f"[MOTOR] Invalid amount for dispensing: {amount}")
            return

        print(f"[MOTOR] Dispensing cash: PHP {amount}")
        self.motor.dispense_amount(amount)

    # -----------------------------
    # Coordinator state setter
    # -----------------------------
